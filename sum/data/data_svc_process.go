...
func (p *process) InitMongoDB(){
	...
	var rreq sumpb.SumRequest
	rreq.Numbers = &sumpb.Numbers{
		A:                    100,
		B:                    200,
	}
...
func makeRedisKey(req *sumpb.SumRequest) string {
	//return fmt.Sprintf("%d_%d", req.Numbers.A, req.Numbers.B)
	return fmt.Sprintf("%d", req.Numbers.A)
}

// Add returns sum of two integers
func (p *process) Add(ctx context.Context, req *sumpb.SumRequest) ( *sumpb.SumResponse, error) {
//todo get jwt token from ctx
	var resp sumpb.SumResponse
	...
	return &sumpb.SumResponse{Result: reqRecord.B}, nil
}
